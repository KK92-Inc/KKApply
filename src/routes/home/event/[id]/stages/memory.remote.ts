...
export const current = query(id, async (userEventId) => {
	...
	if (!row) { // No record yet — generate the initial stage and persist it.
		...
		const data = { $type: "memory", stages: [stage] } satisfies MemoryStore;
		await sql`
	...
	const store = JSON.parse(row.payload) as MemoryStore;
...
/** Submits an answer for the current memory stage. */
