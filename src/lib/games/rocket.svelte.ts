...
export type Puzzle = string & { readonly __brand: 'puzzle' };

export type FunctionSlotInstruction =
	| { type: Exclude<InstructionType, "INVOKE">; color?: Color }
	| { type: "INVOKE"; index: number; color?: Color };
...
export function puzzle(map: string): Puzzle {
	return map as Puzzle;
}

// ============================================================================

/**
 * Procedurally generates levels for the memory game.
 *
 * The generator uses a seeded pseudo-random number generator (Alea) to construct
 * a path from 'S' to 'E' on a grid, while placing color cells and function calls
 * along the way. The difficulty parameter controls the required path length,
 * number of turns, and complexity of the function call graph.
 */
export class Generator {
	...
	public generate(difficulty: number): Level {
		...
		for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
			...
			this.construct(0, functions, state, MAX_STEPS);
			...
			const version: Level = {
				...
				map: puzzle(new TextDecoder().decode(buffer)),
			};
		...
		for (const inst of functions[index] ?? []) {
			...
			switch (inst.type) {
				...
				case "INVOKE":
					...
					this.construct(instance.index, functions, state, maxSteps);
					break;
			}
		}
	}
		...
		for (let fi = 0; fi < fns.length; fi++) {
			...
			outer: for (const caller of fns) {
				for (let ii = 0; ii < caller.length; ii++) {
					...
					if (inst.type !== "INVOKE" || inst.index !== fi) continue;
	...
	private generateInstruction(
		...
		const prevInvokeColor = prevIsInvoke && !!tail.color;
		...
		const target = (fnIndex + 1) % total;
		const priorInvoke = fn.find((inst) => inst.type === "INVOKE" && inst.index === target);
		if (priorInvoke && !priorInvoke.color) {
			options = options.filter(o => o !== "INVOKE");
		}
		...
		if (this.isTurn(prev1 ?? null)) {
			options = options.filter(o => !this.isTurn(o));
		}
