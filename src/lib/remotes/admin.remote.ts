...
export const is = query(v.pipe(v.string(), v.nonEmpty()), async (id) => {
...
export const getDailySignups = query(async () => {
	// SQLite doesn't have a generate_series, so we ask for the raw rows
	// and fill gaps client-side in the component
	return await sql<DailySignup[]>`
		SELECT
